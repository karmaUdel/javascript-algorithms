...
export default class AvlTree extends BinarySearchTree {
  ...
  * 1) Perform standard BST delete for node.
  * 2) Starting from node, travel up and find the first unbalanced node. Let z be the first unbalanced node, y be the larger height child of z, and x be the larger height child of y. Note that the definitions of x and y are different from insertion here.
  ...
  */
	// remove the node treating the node as BST node
	super.remove(node);
	// get parent of deleted node
	let currentNode = this.root.find(node.parent.value);
    // start balancing
	while (currentNode) {
      this.balance(currentNode);
      currentNode = currentNode.parent;
